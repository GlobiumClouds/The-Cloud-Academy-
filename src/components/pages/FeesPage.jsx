...
import { downloadBlob } from '@/lib/download';
import { generateBulkFeeVouchersPdfBlob } from '@/lib/pdf/feeVoucherPdf';
import { feeVoucherService, academicYearService, classService } from '@/services';
...
const FEE_TYPE_OPTS = [
  { value: '__all__', label: 'All Fee Types' },
  { value: 'monthly', label: 'Monthly Fee' },
  { value: 'annual', label: 'Annual Fee' },
  { value: 'lab', label: 'Lab Charges' },
  { value: 'admission', label: 'Admission Fee' },
...
export default function FeesPage() {
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      ...
      const filters = {
        academic_year_id: (isClassMode || isInstituteMode) ? bulkFilters.academicYearId : undefined,
        class_id: bulkFilters.classId && bulkFilters.classId !== '__all__' ? bulkFilters.classId : undefined,
        section_id: bulkFilters.sectionId || undefined,
        month: voucherMonth ? parseInt(voucherMonth, 10) : undefined,
        fee_type: bulkFilters.feeType && bulkFilters.feeType !== '__all__' ? bulkFilters.feeType : undefined,
      };
      ...
      const selectedClassLabel =
        modalClassOptions.find((item) => String(item.value) === String(bulkFilters.classId))?.label || 'class';
      const selectedSectionLabel =
        bulkSectionOptions.find((item) => String(item.value) === String(bulkFilters.sectionId))?.label || 'all-sections';
      const classNameById = new Map(
        (bulkClasses || []).map((item) => [String(item?.id || item?.class_id || ''), item?.name || item?.class_name || ''])
      );
      const sectionNameById = new Map(
        (bulkClasses || [])
          .flatMap((item) => (Array.isArray(item?.sections) ? item.sections : (Array.isArray(item?.Sections) ? item.Sections : [])))
          .map((section) => [String(section?.id || section?.section_id || ''), section?.name || section?.section_name || ''])
      );
      ...
      const selectedFeeType = bulkFilters.feeType && bulkFilters.feeType !== '__all__' ? bulkFilters.feeType : undefined;
      ...
      const vouchersForPdf = vouchersList.map((voucher) => ({
        ...
        className:
          ...
          classNameById.get(String(voucher?.classId || voucher?.class_id || '')) ||
          (bulkFilters.classId && bulkFilters.classId !== '__all__' ? selectedClassLabel : '') ||
          'N/A',
        class_name:
          normalizeDisplayValue(voucher?.class_name) ||
          normalizeDisplayValue(voucher?.className) ||
      ...
      const safeName = `bulk-fee-vouchers-${bulkDownloadMode}-${selectedClassLabel}-${selectedSectionLabel}-${voucherMonth || 'all-months'}`
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <SelectField
              label="Fee Type"
              options={FEE_TYPE_OPTS}
              value={String(bulkFilters.feeType || '__all__')}
              onChange={(val) => setBulkFilters((prev) => ({ ...prev, feeType: val }))}
            />
            <DatePickerField
              label="Due Date"
          ...
          <p className="text-xs text-muted-foreground">
            Downloaded vouchers use the currently selected Month filter and include matching records for selected class/institute.
          </p>
          <div className="flex justify-end gap-2">
            <button
              type="button"
