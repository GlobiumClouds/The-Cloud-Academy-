      ...
      {!value ? (
        ...
        <div
          ...
          className={`
            ...
            flex flex-col items-center justify-center gap-3 transition-all duration-200 group
          ...
          {processing ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-xs font-bold text-primary animate-pulse">
                {resize ? 'Resizing & processing...' : 'Processing...'}
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="text-center space-y-1.5 px-4">
                ...
                {resize && (
                  <p className="text-[10px] font-bold text-violet-500">
                    🔄 Will auto-resize to {resizeWidth}{resizeHeight ? `×${resizeHeight}` : 'px'} at {Math.round(resizeQuality * 100)}% quality
                  </p>
                )}
              </div>
        ...
        /* ── Preview Mode ── */
        <div className={`relative w-full ${aspectRatio} rounded-2xl overflow-hidden border-2 border-slate-200 shadow-sm group`}>
          {isImage ? (
          ...
          {/* Hover overlay */}
          <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/50 transition-all flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
            <button
              type="button"
              onClick={() => !disabled && !processing && inputRef.current?.click()}
              className="flex items-center gap-1.5 bg-white text-slate-800 font-bold text-xs px-3 py-2 rounded-xl shadow-xl hover:bg-slate-100 transition-colors"
            >
            ...
            {onClear && !processing && (
              <button
                type="button"
                onClick={onClear}
                className="flex items-center gap-1.5 bg-red-500 text-white font-bold text-xs px-3 py-2 rounded-xl shadow-xl hover:bg-red-600 transition-colors"
              >
