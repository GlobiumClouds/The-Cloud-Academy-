  ...
  return [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="cursor-pointer max-w-[200px]" onClick={() => router.push(`/master-admin/schools/${s.id}`)}>
    ...
    <ConfirmDialog
      ...
      description={
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">Change status of <strong>{target?.institute_name}</strong>:</p>
          <select
            value={status}
...
function InstituteFormModal({ open, onClose, institute, onSubmit, loading, typeOptions }) {
  ...
  useEffect(() => { if (open) reset(toDefaults(institute ?? null)); }, [institute, open]);
  ...
  const handleClose = () => { reset(toDefaults(null)); onClose(); };
    ...
    <AppModal
      ...
      footer={
        <div className="flex justify-end gap-2 w-full">
          ...
          <Button onClick={handleSubmit(onSubmit)} disabled={loading} className="min-w-[140px] gap-1.5">
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-0.5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          />
          <InputField
            label="Institute Logo URL" name="institute_logo_url" register={register}
            placeholder="https://cloudinary.com/…/logo.png"
            hint="Paste Cloudinary / image URL for institute logo"
          />
        </div>
